    ...
    if (req.isReqCacheable()) {
      ...
      if (cache->isInCache(cacheKey)) {
        ...
        // MUST_REVALIDATE
          ...
          string content = "in cache, but expired at " + cacheRes.getExpireDate();
          writeLog(content);
          sendServer();
          return;
...
void ClientHandler::sendServer() {
  ...
  if (recvResponse() <= 0) {
    std::cerr << "Bad Response from server." << endl;
    send502();
    return;
  }
  ...
  if (req.isReqStoreable() && res.isRespStoreable()) {
    int result = res.isRespCacheable();
    if (result == NO_VALIDATION_INFO) {
      //ID: cached, but requires re-validation
      writeLog("cached, but requires re-validation");
    }
    else if (result == VALID) {
      // ID: cached, expires at EXPIRES
      string content = "cached, expires at " + res.getExpireDate();
      writeLog(content);
    }

    cache->put(cacheKey, res);
  }
  // can not put into cache
  else {
    // ID: not cacheable because REASON
    writeLog("not cacheable because response or request contain no-store");
  }
...
void ClientHandler::send502() {
  const char * status502 = "HTTP/1.1 502 Bad Gateway\r\n\r\n";
  if (send(srcFd, status502, strlen(status502), 0) <= 0) {
    return;
  }
  D("Succeed to send 502 back to client");
}
